                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-zinc-800">
                      <th className="pb-4 font-semibold text-zinc-300">Condition Name</th>
                      <th className="pb-4 font-semibold text-zinc-300 text-right">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {conditions.map((condition) => (
                      <tr key={condition.id} className="border-b border-zinc-800/50 hover:bg-zinc-800/30 transition-colors duration-200">
                        <td className="py-4 font-medium text-zinc-200">{condition.name}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/60 backdrop-blur-sm transition-opacity duration-300">
          <div className="bg-zinc-900/80 backdrop-blur-xl border border-zinc-700 w-full max-w-md rounded-3xl shadow-2xl p-8 transform transition-all scale-100">
            ...
            <form onSubmit={handleSubmit}>
              <div className="mb-6">
                <label className="block text-sm font-medium text-zinc-300 mb-2">Condition Name</label>
                <input
                  type="text"
                  required
                  value={formName}
